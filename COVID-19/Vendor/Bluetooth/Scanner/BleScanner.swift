...
class BleScanner: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate, Scanner {
    ...
    private func handle(event: ProteGoDeviceEvent, for device: ProteGoDevice) {
        print("Handling event: \(event) for: \(device.getId())")
    ...
    private func deviceFound(peripheral: CBPeripheral, rssi: Int?, manufacturerData: Data?) {
        ...
        print("Device found: \(debugDeviceId) rssi: \(debugRSSI) manu: \(debugData)")
    ...
    private func checkSynchronizationStatus() {
        print("Check synchronization status")
    ...
    private func startSynchronizationIfNeeded() {
        ...
        let sortedDevices = self.devices.values.sorted { (lhs, rhs) in
            lhs.hasHigherPriorityForConnection(other: rhs)
        }
        ...
        for index in 0..<freeSlots where index < sortedDevices.count {
            let device = sortedDevices[index]
    ...
    private func startScanningIfNeeded() {
        ...
        case .disabled:
            ...
            print("Scanning failed to start as scanner is disabled")
            return
        case .enabledAllTime:
    ...
    private func stopScanningIfNeeded() {
        ...
        case .enabledAllTime:
            ...
            print("Scanner failed to stop as it's forced to be turned on")
            return
    ...
    func centralManager(_ central: CBCentralManager, willRestoreState dict: [String: Any]) {
        print("CentralManager restored state")
    ...
    func centralManagerDidUpdateState(_ central: CBCentralManager) {
        if central.state == .poweredOn {
            ...
            print("PoweredOff: \(central.state.rawValue)")
    ...
    func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        print("CentralManager did connect: \(peripheral.identifier)")
    ...
    func centralManager(_ central: CBCentralManager, didFailToConnect peripheral: CBPeripheral, error: Error?) {
        print("CentralManager did fail to connect: \(peripheral.identifier) error: \(String(describing: error))")
        if let device = self.getDeviceBy(peripheral: peripheral) {
    ...
    func centralManager(_ central: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) {
        print("CentralManager did disconnect peripheral \(peripheral.identifier) error: \(String(describing: error))")
        if let device = self.getDeviceBy(peripheral: peripheral) {
            handle(event: .disconnected(peripheral, error), for: device)
        }
    }
    ...
    func centralManager(_ central: CBCentralManager,
                        ...
                        rssi RSSI: NSNumber) {
        print("CentralManager did discover \(peripheral.identifier) rssi: \(RSSI)")
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        print("Peripheral did discover services: \(peripheral.identifier), error: \(String(describing: error))")
        if let device = self.getDeviceBy(peripheral: peripheral) {
    ...
    func peripheral(_ peripheral: CBPeripheral, didModifyServices invalidatedServices: [CBService]) {
        print("Peripheral did modify services: \(peripheral.identifier)")
        if let device = self.getDeviceBy(peripheral: peripheral) {
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        print("Peripheral did discover characteristics: " +
    ...
    func peripheral(_ peripheral: CBPeripheral, didReadRSSI RSSI: NSNumber, error: Error?) {
        print("Peripheral did read RSSI: " +
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        print("Peripheral did read value: \(peripheral.identifier), error: \(String(describing: error))")
        if let device = self.getDeviceBy(peripheral: peripheral),
    ...
    func peripheral(_ peripheral: CBPeripheral, didWriteValueFor characteristic: CBCharacteristic, error: Error?) {
        print("Peripheral did write value: \(peripheral.identifier), error: \(String(describing: error))")
        if let device = self.getDeviceBy(peripheral: peripheral),
               characteristic.uuid == Constants.Bluetooth.ProteGOCharacteristicUUID {
